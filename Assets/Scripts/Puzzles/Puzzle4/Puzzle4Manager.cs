...
public class Puzzle4Manager : MonoBehaviour
{
    ...
    private void SpawnLevel()
    {
        ...
        _bgSprite.size = new Vector2(2 * width, 2 * height);
        Vector3 bgPos = new Vector3(
            2 * (width / 2f - _cellSize / 2f - _levelGap / 2f),
            2 * (height / 2f - _cellSize / 2f - _levelGap / 2f),
            0
            );
        ...
        Vector3 rightOffset = Vector3.right * (_cellSize + _cellGap) * 2;
        Vector3 topOffset = Vector3.up * (_cellSize + _cellGap) * 2;
